...
class val Channel is Jsonable
    ...
    let last_message_id: (Snowflake | None)
        ...
        GUILD_FORUM or GUILD_MEDIA channels) (may not point to an existing or
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        var permission_overwrites': (Array[PermissionOverwrite] val | None) =
            None
        ...
        for (key, value) in obj.pairs() do
            ...
            | "permission_overwrites" =>
                permission_overwrites' = _PermissionOverwrites(value)?
            | "name" =>
            ...
            | "last_message_id" =>
                match value
                | let string: String =>
                    last_message_id' = Snowflake.from_json(string)?
                end
            | "bitrate" => bitrate' = (value as I64).usize()
            | "user_limit" => user_limit' = (value as I64).usize()
            | "rate_limit_per_user" =>
                rate_limit_per_user' = (value as I64).usize()
            ...
            | "parent_id" =>
                match value
                | let string: String =>
                    parent_id' = Snowflake.from_json(string)?
                end
            | "last_pin_timestamp" =>
                match value
            ...
            | "video_quality_mode" =>
                video_quality_mode' =
                    VideoQualityModes.from((value as I64).u8())?
            | "message_count" => message_count' = (value as I64).usize()
            | "member_count" => member_count' = (value as I64).usize()
            | "thread_metadata" =>
                thread_metadata' =
                    ThreadMetadata.from_json(value as json.JsonObject)?
            | "member" =>
                member' = ThreadMember.from_json(value as json.JsonObject)?
            | "default_auto_archive_duration" =>
                default_auto_archive_duration' = (value as I64).usize()
            | "permissions" => permissions' = _Permissions(value)?
            | "flags" => flags' = _ChannelFlags((value as I64).u64())
            | "total_message_sent" =>
                total_message_sent' = (value as I64).usize()
            | "available_tags" => available_tags' = _ForumTags(value)?
            | "applied_tags" => applied_tags' = _Snowflakes(value)?
            | "default_reaction_emoji" =>
                match value
                | let obj': json.JsonObject =>
                    default_reaction_emoji' = DefaultReaction.from_json(obj')?
                end
            | "default_thread_rate_limit_per_user" =>
                default_thread_rate_limit_per_user' = (value as I64).usize()
            | "default_sort_order" =>
                match value
                | let integer: I64 =>
                    default_sort_order' = SortOrderTypes.from(integer.u8())?
                end
            | "default_forum_layout" =>
                default_forum_layout' =
                    ForumLayoutTypes.from((value as I64).u8())?
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let last_message_id': Snowflake =>
            obj = obj.update("last_message_id", last_message_id'.to_json())
        end
        ...
        match recipients
        | let recipients': Array[User] val =>
            obj = obj.update("recipients", _Users.to_json(recipients'))
        end
        ...
        | let message_count': USize =>
            obj = obj.update("message_count", message_count'.i64())
        end

        match member_count
        | let member_count': USize =>
            obj = obj.update("member_count", member_count'.i64())
        end

        match thread_metadata
        | let thread_metadata': ThreadMetadata =>
            obj = obj.update("thread_metadata", thread_metadata'.to_json())
        end
        ...
        match flags
        | let flags': Array[ChannelFlag] val =>
            obj = obj.update("flags", _ChannelFlags.to_json(flags'))
        end

        match total_message_sent
        | let total_message_sent': USize =>
            obj = obj.update("total_message_sent", total_message_sent'.i64())
        end

        match available_tags
        | let available_tags': Array[ForumTag] val =>
            obj =
                obj.update(
                    "available_tags", _ForumTags.to_json(available_tags')
                )
        end
    ...
    fun apply(value: json.JsonValue): Array[Channel] val ? =>
        ...
        recover val
            ...
            for channel in array.values() do
                channels.push(Channel.from_json(channel as json.JsonObject)?)
            end
            channels
        end
...
class val PartialChannel is Jsonable
    ...
    A channel Discord sent as a *partial* object: the same structure as
    `Channel`, but carrying only some of its fields. Invites, webhooks,
    interactions and guild widgets all embed channels this way, and none of them
    agree on which fields they include — a guild widget omits even `type` — so
...
primitive _PartialChannels
    fun apply(value: json.JsonValue): Array[PartialChannel] val ? =>
        ...
        recover val
            ...
            for channel in array.values() do
                channels.push(
                    PartialChannel.from_json(channel as json.JsonObject)?
                )
            end
            channels
        end
    ...
    fun apply(value: json.JsonValue): Array[ChannelType] val ? =>
        ...
        recover val
            ...
            for type' in array.values() do
                types.push(ChannelTypes.from((type' as I64).u8())?)
            end
            types
        end
...
primitive PinnedChannelFlag is ChannelFlag
    """
    this thread is pinned to the top of its parent GUILD_FORUM or GUILD_MEDIA
    channel
    """
...
primitive RequireTagChannelFlag is ChannelFlag
    ...
    GUILD_FORUM or a GUILD_MEDIA channel. Tags are specified in the applied_tags
    field.
    """
...
primitive HideMediaDownloadOptionsChannelFlag is ChannelFlag
    """
    when set hides the embedded media download options. Available only for media
    channels
    """
...
class val PermissionOverwrite is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "id" => id' = Snowflake.from_json(value)?
            | "type" =>
                type'' = PermissionOverwriteTypes.from((value as I64).u8())?
...
primitive _PermissionOverwrites
    fun apply(value: json.JsonValue): Array[PermissionOverwrite] val ? =>
        ...
        recover val
            ...
            for overwrite in array.values() do
                overwrites.push(
                    PermissionOverwrite.from_json(overwrite as json.JsonObject)?
                )
            end
            overwrites
        end
    ...
    fun to_json(overwrites: Array[PermissionOverwrite] val): json.JsonArray =>
        var array = json.JsonArray
        for overwrite in overwrites.values() do
            array = array.push(overwrite.to_json())
        end
        array
...
class val ThreadMetadata is Jsonable
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let create_timestamp': ISO8601 =>
            obj = obj.update("create_timestamp", create_timestamp')
        end
...
class val ThreadMember is Jsonable
    ...
    let member: (GuildMember | None)
        ...
        Only included when `with_member` is set to true when calling List Thread
        Members or Get Thread Member.
        """
    ...
    let presence: (Presence | None)
        ...
        Only included on the members added in the THREAD_MEMBERS_UPDATE event,
        where Discord may also send it as `null`.
        """
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "presence" =>
                match value
                | let obj': json.JsonObject =>
                    presence' = Presence.from_json(obj')?
                end
            end
        end
    ...
    fun apply(value: json.JsonValue): Array[ThreadMember] val ? =>
        ...
        recover val
            ...
            for member in array.values() do
                members.push(ThreadMember.from_json(member as json.JsonObject)?)
            end
            members
        end
...
class val ForumTag is Jsonable
    ...
    When updating a GUILD_FORUM or a GUILD_MEDIA channel, tag objects in
    `available_tags` only require the `name` field.
    """
...
primitive _ForumTags
    fun apply(value: json.JsonValue): Array[ForumTag] val ? =>
        ...
        recover val
            ...
            for forum_tag in array.values() do
                tags.push(ForumTag.from_json(forum_tag as json.JsonObject)?)
            end
            tags
        end
    ...
    fun to_json(tags: Array[ForumTag] val): json.JsonArray =>
        var array = json.JsonArray
        for forum_tag in tags.values() do
            array = array.push(forum_tag.to_json())
        end
        array
...
class val ForumTagParams is ToJsonable
    ...
    A tag to be applied to a `GUILD_FORUM` or `GUILD_MEDIA` channel's
    `available_tags`.

    Unlike the forum tag objects the API returns, only `name` is required here:
    `id` identifies an existing tag to keep or rename, and is omitted to create
    a new one.
    """
...
class val UpdateChannelParams is ToJsonable
    ...
    The fields accepted depend on the channel being modified. Group DM channels
    accept only `name` and `icon`. Threads accept `name`, `archived`,
    `auto_archive_duration`, `locked`, `invitable`, `rate_limit_per_user`,
    `flags` and `applied_tags`. All other guild channels accept the remaining
    fields.
    ...
    let user_limit: Nullable[USize]
        """
        the user limit of the voice or stage channel, max 99 for voice channels
        and 10000 for stage channels (0 refers to no limit)
        """
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let auto_archive_duration': USize =>
            obj =
                obj.update(
                    "auto_archive_duration", auto_archive_duration'.i64()
                )
        end
...
class val UpdateChannelPermissionsParams is ToJsonable
    ...
    fun to_json(): json.JsonObject =>
        ...
        match allow
        | let allow': Array[Permission] val =>
            obj = obj.update("allow", _Permissions.to_json(allow'))
        | Null => obj = obj.update("allow", None)
        end

        match deny
        | let deny': Array[Permission] val =>
            obj = obj.update("deny", _Permissions.to_json(deny'))
...
class val CreateChannelInviteParams is ToJsonable
    ...
    All JSON parameters for this route are optional, however the request body is
    not. If you are not sending any fields, you still have to send an empty JSON
    object (`{}`).
    """
    ...
    let max_age: (USize | None)
        ...
        and 604800 (7 days). Defaults to 86400 (24 hours)
        """
    ...
    let unique: (Bool | None)
        ...
        unique one time use invites). Defaults to false
        """
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let target_user_id': Snowflake =>
            obj = obj.update("target_user_id", target_user_id'.to_json())
        end

        match target_application_id
        | let target_application_id': Snowflake =>
            obj =
                obj.update(
                    "target_application_id", target_application_id'.to_json()
                )
        end
...
class val GetThreadMembersParams
    ...
    Requires the `GUILD_MEMBERS` privileged intent to be enabled for the
    application.
    """
