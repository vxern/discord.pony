...
class val Poll is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            match key
            | "question" => question' = PollMedia.from_json(value as json.JsonObject)?
            ...
            | "layout_type" => layout_type' = PollLayoutTypes.from((value as I64).u8())?
            | "results" => results' = PollResults.from_json(value as json.JsonObject)?
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        match results
        | let results': PollResults => obj = obj.update("results", results'.to_json())
        end
...
class val PollAnswer is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "poll_media" => poll_media' = PollMedia.from_json(value as json.JsonObject)?
            end
        end
...
primitive _PollAnswers
    fun apply(value: json.JsonValue): Array[PollAnswer] val ? =>
        ...
        recover val
            ...
            for answer in array.values() do answers.push(PollAnswer.from_json(answer as json.JsonObject)?) end
            answers
        end
...
class val PollResults is Jsonable
    ...
    Due to the intricacies of counting at scale, while a poll is in progress the results may not be perfectly accurate. They usually are accurate, and shouldn't deviate significantly -- it's just difficult to make guarantees.

    To compensate for this, after a poll is finished there is a background job which performs a final, accurate tally of votes. This tally has concluded once `is_finalized` is true.
...
primitive _PollAnswerCounts
    fun apply(value: json.JsonValue): Array[PollAnswerCount] val ? =>
        ...
        recover val
            ...
            for count in array.values() do counts.push(PollAnswerCount.from_json(count as json.JsonObject)?) end
            counts
        end
