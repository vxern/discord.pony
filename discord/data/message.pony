...
class val Message is Jsonable
    ...
    and `components` fields while `poll` will be omitted if they have not
    configured (or been approved for) the `MESSAGE_CONTENT` privileged intent
    (`1 << 15`).
    """
    ...
    let application: (PartialApplication | None)
        ...
        This is a partial application object: Discord sends only `id`, `name`,
        `icon`, `description` and `cover_image`.
        """
    ...
    let message_snapshots: (Array[MessageSnapshot] val | None)
        """
        the message associated with the `message_reference`. This is a minimal
        subset of fields in a message (e.g. `author` is excluded.)
        """
    ...
    let referenced_message: (Message | None)
        ...
        `21` (THREAD_STARTER_MESSAGE), or `23` (CONTEXT_MENU_COMMAND). If the
        message is one of these but the `referenced_message` field is not
        present, the backend did not attempt to fetch the message that was being
        replied to, so its state is unknown. If the field exists but is null,
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "edited_timestamp" =>
                match value
                | let string: String => edited_timestamp' = string
                end
            ...
            | "activity" =>
                activity' = MessageActivity.from_json(value as json.JsonObject)?
            | "application" =>
                application' =
            ...
            | "message_reference" =>
                message_reference' =
                    MessageReference.from_json(value as json.JsonObject)?
            | "message_snapshots" =>
                message_snapshots' = _MessageSnapshots(value)?
            | "referenced_message" =>
                match value
                | let obj': json.JsonObject =>
                    referenced_message' = Message.from_json(obj')?
                end
            | "interaction_metadata" =>
                interaction_metadata' =
                    MessageInteractionMetadatas.from_json(
                        value as json.JsonObject
                    )?
            | "interaction" =>
                interaction' =
                    MessageInteraction.from_json(value as json.JsonObject)?
            ...
            | "role_subscription_data" =>
                role_subscription_data' =
                    RoleSubscriptionData.from_json(value as json.JsonObject)?
            | "resolved" =>
            ...
            | "shared_client_theme" =>
                shared_client_theme' =
                    SharedClientTheme.from_json(value as json.JsonObject)?
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let mention_channels': Array[ChannelMention] val =>
            obj =
                obj.update(
                    "mention_channels",
                    _ChannelMentions.to_json(mention_channels')
                )
        end

        match reactions
        | let reactions': Array[MessageReaction] val =>
            obj = obj.update("reactions", _MessageReactions.to_json(reactions'))
        end
        ...
        | let webhook_id': Snowflake =>
            obj = obj.update("webhook_id", webhook_id'.to_json())
        end

        match activity
        | let activity': MessageActivity =>
            obj = obj.update("activity", activity'.to_json())
        end
        ...
        | let message_reference': MessageReference =>
            obj = obj.update("message_reference", message_reference'.to_json())
        end

        match message_snapshots
        | let message_snapshots': Array[MessageSnapshot] val =>
            obj =
                obj.update(
                    "message_snapshots",
                    _MessageSnapshots.to_json(message_snapshots')
                )
        end
        ...
        | let referenced_message': Message =>
            obj =
                obj.update("referenced_message", referenced_message'.to_json())
        end

        match interaction_metadata
        | let interaction_metadata': MessageInteractionMetadata =>
            obj =
                obj.update(
                    "interaction_metadata", interaction_metadata'.to_json()
                )
        end

        match interaction
        | let interaction': MessageInteraction =>
            obj = obj.update("interaction", interaction'.to_json())
        end
        ...
        match sticker_items
        | let sticker_items': Array[StickerItem] val =>
            obj =
                obj.update(
                    "sticker_items", _StickerItems.to_json(sticker_items')
                )
        end
        ...
        | let role_subscription_data': RoleSubscriptionData =>
            obj =
                obj.update(
                    "role_subscription_data", role_subscription_data'.to_json()
                )
        end
        ...
        | let shared_client_theme': SharedClientTheme =>
            obj =
                obj.update(
                    "shared_client_theme", shared_client_theme'.to_json()
                )
        end
    ...
    fun apply(value: json.JsonValue): Array[Message] val ? =>
        ...
        recover val
            ...
            for message in array.values() do
                messages.push(Message.from_json(message as json.JsonObject)?)
            end
            messages
        end
...
trait val MessageType is _Enum[MessageType, U8]
    ...
    Type `19` and `20` are only available in API v8 and above. In v6, they are
    represented as type `0`. Additionally, type `21` is only available in API v9
    and above.
    """
...
primitive IsComponentsV2MessageFlag is MessageFlag
    ...
    Once a message has been sent with this flag, it can't be removed from that
    message.
    """
...
//
// Metadata about the interaction, including the source of the interaction and
// relevant server and user IDs.
type MessageInteractionMetadata is (
...
primitive MessageInteractionMetadatas
    fun from_json(obj: json.JsonObject): MessageInteractionMetadata ? =>
        ...
        | ApplicationCommandInteractionType =>
            ApplicationCommandInteractionMetadata.from_json(obj)?
        | MessageComponentInteractionType =>
            MessageComponentInteractionMetadata.from_json(obj)?
        | ModalSubmitInteractionType =>
            ModalSubmitInteractionMetadata.from_json(obj)?
        else error
        end
...
class val ApplicationCommandInteractionMetadata is Jsonable
    ...
    let target_message_id: (Snowflake | None)
        """
        The ID of the message the command was run on, present only on message
        command interactions. The original response message will also have
        `message_reference` and `referenced_message` pointing to this message.
        """
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "original_response_message_id" =>
                original_response_message_id' = Snowflake.from_json(value)?
            | "target_user" =>
                target_user' = User.from_json(value as json.JsonObject)?
            | "target_message_id" =>
                target_message_id' = Snowflake.from_json(value)?
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let original_response_message_id': Snowflake =>
            obj =
                obj.update(
                    "original_response_message_id",
                    original_response_message_id'.to_json()
                )
        end
        ...
        | let target_message_id': Snowflake =>
            obj = obj.update("target_message_id", target_message_id'.to_json())
        end
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "interacted_message_id" =>
                interacted_message_id' = Snowflake.from_json(value)?
            end
        end
...
class val ModalSubmitInteractionMetadata is Jsonable
    ...
    let triggering_interaction_metadata: (
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        var authorizing_integration_owners': (
            ...
            None
        var original_response_message_id': (Snowflake | None) = None
        var triggering_interaction_metadata': (
            ApplicationCommandInteractionMetadata
            | MessageComponentInteractionMetadata | None
        ) =
            None
        ...
        original_response_message_id = original_response_message_id'
        triggering_interaction_metadata =
            triggering_interaction_metadata' as (
                ApplicationCommandInteractionMetadata
                | MessageComponentInteractionMetadata
            )
    ...
    fun to_json(): json.JsonObject =>
        var obj = json.JsonObject
            ...
            .update(
                "triggering_interaction_metadata",
                triggering_interaction_metadata.to_json()
            )
...
class val MessageInteraction is Jsonable
    ...
    Deprecated in favour of `MessageInteractionMetadata`. This is sent on the
    message object when the message is a response to an interaction without an
    existing message.
    """
    ...
    let member: (PartialGuildMember | None)
        ...
        This is a partial guild member object, so most of its fields may be
        absent.
        """
...
class val MessageCall is Jsonable
    ...
    new val create(
        participants': Array[Snowflake] val,
        ended_timestamp': (ISO8601 | None) = None
    ) =>
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "ended_timestamp" =>
                match value
                | let string: String => ended_timestamp' = string
                end
            end
        end
...
class val MessageReference is Jsonable
    ...
    Message references are generic attribution on a message. There are multiple
    message types that have a `message_reference` object.
    """
    ...
    let type': (MessageReferenceType | None)
        ...
        If unset, `DefaultMessageReferenceType` can be assumed in order to match
        the behavior before message reference had types. In future API versions
        this will become a required field.
        """
    ...
    let channel_id: (Snowflake | None)
        ...
        Optional when creating a reply, but always present when receiving an
        event/response that includes this data model. Required for forwards.
        """
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            match key
            | "type" =>
                type'' = MessageReferenceTypes.from((value as I64).u8())?
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let fail_if_not_exists': Bool =>
            obj = obj.update("fail_if_not_exists", fail_if_not_exists')
        end
...
primitive ForwardMessageReferenceType is MessageReferenceType
    ...
    `FORWARD` can only be used for basic messages; i.e. messages which do not
    have strong bindings to a non global entity. Thus Discord supports only
    messages with type `DEFAULT`, `REPLY`, `CHAT_INPUT_COMMAND`, or
    `CONTEXT_MENU_COMMAND`, and does not support messages with a poll, call, or
    activity. This is subject to change in the future.
    """
...
class val MessageSnapshot is Jsonable
    ...
    A snapshot captures a minimal subset of a forwarded message, to prevent
    spoofing of forwarded data. Snapshots are immutable and taken at creation
    time.

    Discord documents the `message` field as a partial message object. Because
    that partial omits fields `Message` requires (`id`, `channel_id`, `author`,
    `tts`, `mention_everyone` and `pinned`), the documented subset is modelled
    here directly rather than as a nested `Message`.

    While message snapshots are able to support nested snapshots, Discord
    currently limits the depth of nesting to 1.
    """
    ...
    fun to_json(): json.JsonObject =>
        ...
        match flags
        | let flags': Array[MessageFlag] val =>
            message = message.update("flags", _MessageFlags.to_json(flags'))
        end

        match stickers
        | let stickers': Array[Sticker] val =>
            message = message.update("stickers", _Stickers.to_json(stickers'))
        end
        ...
        | let components': Array[Component] val =>
            message =
                message.update("components", _Components.to_json(components'))
        end
...
primitive _MessageSnapshots
    fun apply(value: json.JsonValue): Array[MessageSnapshot] val ? =>
        ...
        recover val
            ...
            for snapshot in array.values() do
                snapshots.push(
                    MessageSnapshot.from_json(snapshot as json.JsonObject)?
                )
            end
            snapshots
        end
    ...
    fun to_json(snapshots: Array[MessageSnapshot] val): json.JsonArray =>
        ...
        for snapshot in snapshots.values() do
            array = array.push(snapshot.to_json())
        end
        array
...
class val MessageReaction is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "count_details" =>
                count_details' =
                    MessageReactionCountDetails.from_json(
                        value as json.JsonObject
                    )?
            | "me" => me' = value as Bool
...
primitive _MessageReactions
    fun apply(value: json.JsonValue): Array[MessageReaction] val ? =>
        ...
        recover val
            ...
            for reaction in array.values() do
                reactions.push(
                    MessageReaction.from_json(reaction as json.JsonObject)?
                )
            end
            reactions
        end
    ...
    fun to_json(reactions: Array[MessageReaction] val): json.JsonArray =>
        ...
        for reaction in reactions.values() do
            array = array.push(reaction.to_json())
        end
        array
...
class val MessageReactionCountDetails is Jsonable
    ...
    The reaction count details object contains a breakdown of normal and super
    reaction counts for the associated emoji.
    """
...
class val MessageEmbed is Jsonable
    ...
    To facilitate showing rich content, rich embeds do not follow the
    traditional limits of message content. However, some limits are still in
    place to prevent excessively large embeds: `title` 256 characters,
    `description` 4096 characters, up to 25 `fields`, `field.name` 256
    characters, `field.value` 1024 characters, `footer.text` 2048 characters,
    `author.name` 256 characters. Additionally, the combined sum of characters
    in all `title`, `description`, `field.name`, `field.value`, `footer.text`,
    and `author.name` fields across all embeds attached to a message must not
    exceed 6000 characters.

    Embeds are deduplicated by URL. If a message contains multiple embeds with
    the same URL, only the first is shown.
    """
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "footer" =>
                footer' =
                    MessageEmbedFooter.from_json(value as json.JsonObject)?
            | "image" =>
                image' = MessageEmbedImage.from_json(value as json.JsonObject)?
            | "thumbnail" =>
                thumbnail' =
                    MessageEmbedImage.from_json(value as json.JsonObject)?
            | "video" =>
                video' = MessageEmbedVideo.from_json(value as json.JsonObject)?
            | "provider" =>
                provider' =
                    MessageEmbedProvider.from_json(value as json.JsonObject)?
            | "author" =>
                author' =
                    MessageEmbedAuthor.from_json(value as json.JsonObject)?
    ...
    fun to_json(): json.JsonObject =>
        ...
        match type'
        | let type''': MessageEmbedType =>
            obj = obj.update("type", type'''.value())
        end
        ...
        | let footer': MessageEmbedFooter =>
            obj = obj.update("footer", footer'.to_json())
        end

        match image
        | let image': MessageEmbedImage =>
            obj = obj.update("image", image'.to_json())
        end

        match thumbnail
        | let thumbnail': MessageEmbedImage =>
            obj = obj.update("thumbnail", thumbnail'.to_json())
        end

        match video
        | let video': MessageEmbedVideo =>
            obj = obj.update("video", video'.to_json())
        end

        match provider
        | let provider': MessageEmbedProvider =>
            obj = obj.update("provider", provider'.to_json())
        end

        match author
        | let author': MessageEmbedAuthor =>
            obj = obj.update("author", author'.to_json())
        end

        match fields
        | let fields': Array[MessageEmbedField] val =>
            obj = obj.update("fields", _MessageEmbedFields.to_json(fields'))
        end

        match flags
        | let flags': Array[MessageEmbedFlag] val =>
            obj = obj.update("flags", _MessageEmbedFlags.to_json(flags'))
        end
    ...
    fun apply(value: json.JsonValue): Array[MessageEmbed] val ? =>
        ...
        recover val
            ...
            for entry in array.values() do
                embeds.push(MessageEmbed.from_json(entry as json.JsonObject)?)
            end
            embeds
        end
...
trait val MessageEmbedType is _Enum[MessageEmbedType, String]
    ...
    Embed types are "loose" types that should be considered deprecated and might
...
primitive PollResultMessageEmbedType is MessageEmbedType
    ...
    Poll result embeds carry their additional data as fields:
    `poll_question_text`, `victor_answer_votes`, `total_votes`,
    `victor_answer_id`, `victor_answer_text`, `victor_answer_emoji_id`,
    `victor_answer_emoji_name` and `victor_answer_emoji_animated`.
    """
...
class val MessageEmbedVideo is Jsonable
    ...
    fun to_json(): json.JsonObject =>
        ...
        match flags
        | let flags': Array[MessageEmbedMediaFlag] val =>
            obj = obj.update("flags", _MessageEmbedMediaFlags.to_json(flags'))
        end
...
class val MessageEmbedAuthor is Jsonable
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let proxy_icon_url': String =>
            obj = obj.update("proxy_icon_url", proxy_icon_url')
        end
...
primitive _MessageEmbedFields
    fun apply(value: json.JsonValue): Array[MessageEmbedField] val ? =>
        ...
        recover val
            ...
            for field in array.values() do
                fields.push(
                    MessageEmbedField.from_json(field as json.JsonObject)?
                )
            end
            fields
        end
...
class val MessageAttachment is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "application" =>
                match value
                | let obj': json.JsonObject =>
                    application' = Application.from_json(obj')?
                end
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let duration_secs': F64 =>
            obj = obj.update("duration_secs", duration_secs')
        end
        ...
        match flags
        | let flags': Array[MessageAttachmentFlag] val =>
            obj = obj.update("flags", _MessageAttachmentFlags.to_json(flags'))
        end

        match clip_participants
        | let clip_participants': Array[User] val =>
            obj =
                obj.update(
                    "clip_participants", _Users.to_json(clip_participants')
                )
        end

        match clip_created_at
        | let clip_created_at': ISO8601 =>
            obj = obj.update("clip_created_at", clip_created_at')
        end
...
primitive _MessageAttachments
    fun apply(value: json.JsonValue): Array[MessageAttachment] val ? =>
        ...
        recover val
            ...
            for attachment in array.values() do
                attachments.push(
                    MessageAttachment.from_json(attachment as json.JsonObject)?
                )
            end
            attachments
        end
    ...
    fun to_json(attachments: Array[MessageAttachment] val): json.JsonArray =>
        var array = json.JsonArray
        for attachment in attachments.values() do
            array = array.push(attachment.to_json())
        end
        array
...
primitive IsThumbnailMessageAttachmentFlag is MessageAttachmentFlag
    """
    this attachment is the thumbnail of a thread in a media channel, displayed
    in the grid but not on the message
    """
...
primitive _ChannelMentions
    fun apply(value: json.JsonValue): Array[ChannelMention] val ? =>
        ...
        recover val
            ...
            for mention in array.values() do
                mentions.push(
                    ChannelMention.from_json(mention as json.JsonObject)?
                )
            end
            mentions
        end
    ...
    fun to_json(mentions: Array[ChannelMention] val): json.JsonArray =>
        var array = json.JsonArray
        for mention in mentions.values() do
            array = array.push(mention.to_json())
        end
        array
...
class val AllowedMentions is Jsonable
    ...
    Setting the `allowed_mentions` field lets you determine whether users will
    receive notifications when you include mentions in the message content, or
    the content of components attached to that message. This field is always
    validated against your permissions and the presence of said mentions in the
    message, to avoid "phantom" pings where users receive a notification without
    a visible mention in the message. For example, if you want to ping everyone,
    including it in the `allowed_mentions` field is not enough, the mention
    format (`@everyone`) must also be present in the content of the message or
    its components. It is important to note that setting this field **does not**
    guarantee a push notification will be sent, as additional factors can
    influence this:
    - To mention roles and notify their members, the role's `mentionable` field
      must be set to `true`, or the bot must have the `MENTION_EVERYONE`
      permission
    - To mention `@everyone` and `@here`, the bot must have the
      `MENTION_EVERYONE` permission
    - Setting the `SUPPRESS_NOTIFICATIONS` flag when sending a message will
      disable push notifications and only cause a notification badge
    - Users can customize their notification settings through the Discord app,
      which might cause them to only receive a notification badge and no push
      notification
    """
    ...
    fun to_json(): json.JsonObject =>
        ...
        match parse
        | let parse': Array[AllowedMention] val =>
            obj = obj.update("parse", _AllowedMentions.to_json(parse'))
        end
        ...
        | let users': Array[Snowflake] val =>
            obj = obj.update("users", _Snowflakes.to_json(users'))
        end

        match replied_user
        | let replied_user': Bool =>
            obj = obj.update("replied_user", replied_user')
        end
    ...
    fun apply(value: json.JsonValue): Array[AllowedMention] val ? =>
        ...
        recover val
            ...
            for type' in array.values() do
                types.push(AllowedMentionTypes.from(type' as String)?)
            end
            types
        end
...
class val RoleSubscriptionData is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "role_subscription_listing_id" =>
                role_subscription_listing_id' = Snowflake.from_json(value)?
            | "tier_name" => tier_name' = value as String
            | "total_months_subscribed" =>
                total_months_subscribed' = (value as I64).usize()
            | "is_renewal" => is_renewal' = value as Bool
            end
        end

        role_subscription_listing_id =
            role_subscription_listing_id' as Snowflake
    ...
    fun to_json(): json.JsonObject =>
        json.JsonObject
            .update(
                "role_subscription_listing_id",
                role_subscription_listing_id.to_json()
            )
...
primitive _MessagePins
    fun apply(value: json.JsonValue): Array[MessagePin] val ? =>
        ...
        recover val
            ...
            for pin in array.values() do
                pins.push(MessagePin.from_json(pin as json.JsonObject)?)
            end
            pins
        end
...
class val ChannelPins is Jsonable
    ...
    let has_more: Bool
        """
        whether there are potentially additional pins that could be returned on
        a subsequent call
        """
...
class val MessageSearchResults is Jsonable
    ...
    let total_results: USize
        ...
        When messages are actively being created or deleted, this may not be
        accurate.
        """
    ...
    let messages: Array[Array[Message] val] val
        ...
        The nested array was used to provide surrounding context to search
        results. However, surrounding context is no longer returned.
        """
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "doing_deep_historical_index" =>
                doing_deep_historical_index' = value as Bool
    ...
    fun to_json(): json.JsonObject =>
        ...
        | let documents_indexed': USize =>
            obj = obj.update("documents_indexed", documents_indexed'.i64())
        end

        match threads
        | let threads': Array[Channel] val =>
            obj = obj.update("threads", _Channels.to_json(threads'))
        end

        match members
        | let members': Array[ThreadMember] val =>
            obj = obj.update("members", _ThreadMembers.to_json(members'))
        end
...
primitive _MessageGroups
    ...
    fun to_json(groups: Array[Array[Message] val] val): json.JsonArray =>
        ...
        for group in groups.values() do
            array = array.push(_Messages.to_json(group))
        end
        array
...
class val SharedClientTheme is Jsonable
    ...
    new val from_json(obj: json.JsonObject) ? =>
        ...
        for (key, value) in obj.pairs() do
            ...
            | "base_theme" =>
                match value
                | let integer: I64 =>
                    base_theme' = BaseThemeTypes.from(integer.u8())?
                end
            end
        end
    ...
    fun to_json(): json.JsonObject =>
        ...
        match base_theme
        | let base_theme': BaseThemeType =>
            obj = obj.update("base_theme", base_theme'.value().i64())
        end
...
class val MessageAttachmentParams is ToJsonable
    ...
    A partial attachment object, used to keep, describe or rename attachments
...
primitive _MessageAttachmentParams
    fun to_json(
        attachments: Array[MessageAttachmentParams] val
    ): json.JsonArray =>
        var array = json.JsonArray
        for attachment in attachments.values() do
            array = array.push(attachment.to_json())
...
class val SearchGuildMessagesParams
    ...
    let include_nsfw: (Bool | None)
        """
        whether to include messages from age-restricted channels, defaults to
        false
        """
    ...
    fun to_query(): _RequestQuery =>
        ...
        | let author_id': Array[Snowflake] val =>
            for id in author_id'.values() do
                query.push(("author_id", id.string()))
            end
        end

        match channel_id
        | let channel_id': Array[Snowflake] val =>
            for id in channel_id'.values() do
                query.push(("channel_id", id.string()))
            end
        end

        match mentions
        | let mentions': Array[Snowflake] val =>
            for id in mentions'.values() do
                query.push(("mentions", id.string()))
            end
        end
        ...
        match include_nsfw
        | let include_nsfw': Bool =>
            query.push(("include_nsfw", include_nsfw'.string()))
        end
...
class val CreateMessageParams is ToJsonable
    ...
    One of `content`, `embeds`, `sticker_ids`, `components`, `files` or `poll`
    is required.

    Files are uploaded as a `multipart/form-data` body; only the JSON portion of
    such a request is serialised here.
    """
    ...
    let nonce: (String | None)
        ...
        will appear in the Message Create Gateway event.
        """
    ...
    let enforce_nonce: (Bool | None)
        """
        If true and nonce is present, it will be checked for uniqueness in the
        past few minutes. If another message was created by the same author with
        the same nonce, that message will be returned and no new message will be
        created.
        """
    ...
    fun to_json(): json.JsonObject =>
        ...
        match sticker_ids
        | let sticker_ids': Array[Snowflake] val =>
            obj = obj.update("sticker_ids", _Snowflakes.to_json(sticker_ids'))
        end
        ...
        match enforce_nonce
        | let enforce_nonce': Bool =>
            obj = obj.update("enforce_nonce", enforce_nonce')
        end
...
class val ForumThreadMessageParams is ToJsonable
    ...
    The contents of the first message in a forum or media thread. One of
    `content`, `embeds`, `sticker_ids`, `components` or `files` is required.
    """
...
class val GetReactionsParams
    ...
    fun to_query(): _RequestQuery =>
        ...
        | let type'': MessageReactionType =>
            query.push(("type", type''.value().string()))
        end
...
class val UpdateMessageParams is ToJsonable
    ...
    let flags: Nullable[Array[MessageFlag] val]
        """
        Edit the flags of a message (only `SUPPRESS_EMBEDS` and
        `IS_COMPONENTS_V2` can currently be set/unset)
        """
